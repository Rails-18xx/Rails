...
/*
			...
			if (gameName != null && !gameName.equals("")) {
				...
				if (GameManager.getVariants().size() > 1) {
		    ...
		    String varName = request.getParameter("Variant");
		    if (GameManager.existVariant(varName)) {
		        GameManager.setVariant(varName);
		        process = SELECTPLAYERS;
		    } else {
			...
			if (hasValue(request.getParameter("AddPlayer"))) {
				...
				process = BUYPRIVATES; // From here on we will not use process
									   // anymore.
				gameMgr = GameManager.getInstance();
				currentRound = gameMgr.getCurrentRound();
				System.out.println("First round is "
						+ currentRound.getClass().getName());
...
/*
		...
		out.append(".stockmarket0 { background-color: white; border: 1px solid black; text-align:center; vertical-align:top;}\n");
		out.append(".stockmarket1 { background-color: yellow; border: 1px solid black; text-align:center; vertical-align:top;}\n");
		out.append(".stockmarket2 { background-color: orange; border: 1px solid black; text-align:center; vertical-align:top;}\n");
		out.append(".stockmarket3 { color:white; background-color: brown; border: 1px solid black; text-align:center; vertical-align:top;}\n");
		out.append(".stockmarket_start { background-color: white; border: 2px solid red; text-align:center; vertical-align:top;}\n");
		...
		if (process == SELECTGAME) {
			...
			out.append("<form method=\"POST\" action=\"" + servletPrefix
					+ servletName + "\">\n");
			out.append("Select a variant: ");
			List variants = GameManager.getVariants();
			out.append("<select name=\"Variant\" onChange=\"javascript:this.form.submit()\">\n");
			out.append("<option value=\"\">-- select a variant --");
			...
			for (col = 0; col < stockMarket.getNumberOfColumns(); col++) {
				out.append("<td align=\"center\">"
						+ Character.toString((char) ('A' + col)) + "</td>");
			}
			out.append("</tr>\n");
			...
			for (row = 0; row < stockMarket.getNumberOfRows(); row++) {
				...
				for (col = 0; col < stockMarket.getNumberOfColumns(); col++) {
					...
					if (square != null) {
						out.append("<td width=\"30\" height=\"40\" class=\"stockmarket");
						if (square.isStart())
							out.append("_start");
						else if (square.isNoBuyLimit())
							out.append("3");
						else if (square.isNoHoldLimit())
							out.append("2");
						else if (square.isNoCertLimit())
							out.append("1");
						else
							out.append("0");
						out.append("\"");
						...
						while (iterator.hasNext()) {
							...
							out.append("<br><span style=\"color:"
									+ company.getHexFgColour()
									+ ";background-color:"
									+ company.getHexBgColour() + "\">"
									+ company.getName() + "</span>");
						}
						iterator = square.getFixedStartPrices().iterator();
						while (iterator.hasNext()) {
							company = (PublicCompany) iterator.next();
			...
			out
					.append("</td><td valign=\"top\" class=\"bigtable\">\n<h3>Actions</h3>\n");
			...
			if (process == SELECTPLAYERS) {
				...
				if (step == StartRoundI.SET_PRICE) {
					...
					out.append("<h4>").append(player.getName()).append(
							": set par price for company ").append(
							currCo.getName()).append("</h4><p>");
					out.append("<form method=\"POST\" action=\""
							+ servletPrefix + servletName + "\">\n");
					...
					out.append("<h4>Buying Privates - ").append(
							startRound.getCurrentPlayer().getName()).append(
							"'s turn</h4>");
					out.append("<form method=\"POST\" action=\""
							+ servletPrefix + servletName + "\">\n");
					out.append("<input type=hidden name=Player value=\"")
					...
					if (step != StartRound.BUY) {
						out.append("<tr><td align=right>")
						   .append("<input type=submit name=Pass value=\"Pass\"></td></tr>\n");
					}
				...
				out.append("<h4>Stock Round ")
					.append(stockRound.getStockRoundNumber()) .append(", ")
					.append(stockRound.getCurrentPlayer().getName()) 
					.append("'s turn</h4><p>\n");
				...
				out.append("<input type=hidden name=Player value=\"").append(
						stockRound.getCurrentPlayer().getName()).append(
						"\"><table>\n");

				List companies = Game.getCompanyManager()
						.getAllPublicCompanies();

				boolean askStartPrice = true;
				if (stockRound.mayCurrentPlayerBuyAtAll()) {
					out.append("<tr><td align=right><input type=submit name=Start value=\"Start Company\"></td>")
					...
					for (int j = 0; j < companies.size(); j++) {
						...
						if (stockRound.isCompanyStartable(company.getName())) {
							...
							if (company.getParPrice() != null) {
								out.append(" at ") 
								    .append(Bank.format(company.getParPrice().getPrice()));
								askStartPrice = false;
							}
							out.append("\n");
						}
					...
					out.append("</select>\n</td>");
					if (askStartPrice) {
						out.append("<td> at <select name=StartPrice>\n");
						Iterator it = Game.getStockMarket().getStartSpaces()
								.iterator();
						while (it.hasNext()) {
							price = ((StockSpaceI) it.next()).getPrice();
					...
					for (int j = 0; j < companies.size(); j++) {
						company = (PublicCompanyI) companies.get(j);
						if (stockRound
								.isCompanyBuyable(company.getName(), pool)) {
							out.append("<option value=\"") 
							   .append(company.getName()) 
							   .append("\">") 
							   .append(company.getName()) 
				...
				if (stockRound.mayCurrentPlayerSellAtAll()) {
					out.append("<tr><td align=right><input type=submit name=Sell value=\"Sell\">");
					out.append("<select name=Number>\n");
					for (int k = 1; k <= 5; k++)
						out.append("<option value=" + k + ">" + k + "\n");
					out.append("</select></td>\n").append(
							"<td><select name=SellCompany>\n");
					for (int j = 0; j < companies.size(); j++) {
						company = (PublicCompanyI) companies.get(j);
						if (stockRound.isCompanySellable(company.getName())) {
							out.append("<option value=\"").append(
									company.getName()).append("\">").append(
				...
				if (currCo.canBuyPrivates()) {
					...
					while (it.hasNext()) {
						...
						if (priv.getPortfolio().getOwner() instanceof Player) {
							out.append("<option value=\"" + priv.getName() + "\">"
									+ priv.getName() + "\n");
						}
					}
			...
			out
					.append("</td></tr><tr><td valign=\"top\" class=\"bigtable\">\n<h3>Messages</h3>\n");
			String errMsg = Log.getErrorBuffer();
			if (hasValue(errMsg)) {
				out.append("<p><font color=red><b>").append(
						errMsg.replaceAll(":", ":<br>&nbsp;&nbsp;")).append(
						"</b></font>");
			}

			out.append("<p>").append(
					Log.getMessageBuffer().replaceAll("\n", "<br>"));
			...
			if (stockMarket.isGameOver()) {
				...
				while (iterator.hasNext()) {
					...
					if (company.isClosed()) {
						...
						} else if (company.getParPrice() != null) {
							out.append("<td>").append(
									company.hasParPrice() ? Bank.format(company
											.getParPrice().getPrice())
											: "&nbsp;").append("</td><td>")
									.append(
											Bank.format(company
													.getCurrentPrice()
						...
						if (company.hasFloated()) {
							out.append("<td>" + company.getFormattedCash()
									+ "</td><td>"
									+ Bank.format(company.getLastRevenue())
									+ "</td><td>&nbsp;");
							Iterator it = company.getPortfolio()
									.getPrivateCompanies().iterator();
							while (it.hasNext()) {
								out.append(((PrivateCompanyI) it.next())
										.getName()
										+ " ");
							}
						} else {
						...
						out.append("</td><td>"
								+ Bank.getIpo().ownsShare(company));
						out.append("%</td><td>"
								+ Bank.getPool().ownsShare(company));
						out.append("%</td>\n");
					}
					out.append("</tr>\n");
				}
			...
			boolean includeMinors = 
				companyManager.getCompaniesByType("Minor")!= null;
			...
			// NOTE: The below code has hardcoded behaviour for company type
			// "Minor".
			if (includeMinors)
				out.append("<th>Minors</th>");
			List allCompanies = Game.getCompanyManager()
					.getAllPublicCompanies();
			Iterator it = allCompanies.iterator();
			while (it.hasNext()) {
				company = (PublicCompanyI) it.next();
				if (!company.getType().getName().equals("Minor"))
					out.append("<th>" + company.getName() + "</th>");
			}
			out.append("<th>Worth</th></tr>\n");
			for (int j = 0; j < (players != null ? players.length : -1); j++) {
				Player player = players[j];
				out.append("<tr><td>" + player.getName());
				if (player == GameManager.getPriorityPlayer())
					out.append(" *P*");
				out.append("</td><td>" + player.getFormattedCash());
				if (currentRound instanceof StartRoundI
						&& player.getCash() > player.getUnblockedCash()) {
					out.append(" (").append(
							Bank.format(player.getUnblockedCash())).append(")");
				}
				out.append("</td><td>");
				...
				if (includeMinors) {
					out.append("<td>");
					it = player.getPortfolio().getCertificates().iterator();
					while (it.hasNext()) {
						company = ((PublicCertificate) it.next()).getCompany();
						if (company.getType().getName().equals("Minor"))
							out.append(" " + company.getName());
					}
					out.append("&nbsp;</td>");
				...
				while (it.hasNext()) {
					...
					if ((certs = player.getPortfolio()
							.getCertificatesPerCompany(compName)) != null) {
					...
					out.append("<td>" + (president ? "P" : "") + share
							+ "%</td>");
				}
				out.append("<td>").append(player.getFormattedWorth()).append(
						"</td></tr>");
			...
			// End
			out
					.append("</td></tr><tr><td></td><td width=\"10%\"></td><td></td></tr></table>\n");
