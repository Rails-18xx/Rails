/* $Header: /Users/blentz/rails_rcs/cvs/18xx/rails/ui/swing/StatusWindow.java,v 1.21 2008/03/05 19:55:16 evos Exp $*/
...
public class StatusWindow extends JFrame implements ActionListener,
	...
	protected static Logger log = Logger.getLogger(StatusWindow.class
			.getPackage().getName());
	...
	public void init(GameUIManager gameUIManager) {
		...
		Class<? extends GameStatus> gameStatusClass = gameManager
				.getGameStatusClass();
	...
	public void setGameActions() {
		...
		List<GameAction> gameActions = possibleActions
				.getType(GameAction.class);
		if (gameActions != null) {
	...
	public void updateStatus() {
		...
		if (currentRound instanceof TreasuryShareRound) {
			...
			setTitle(LocalText.getText("TRADE_TREASURY_SHARES_TITLE",
					((TreasuryShareRound) currentRound).getOperatingCompany()
							.getName()));
			...
			setTitle(LocalText.getText("EMERGENCY_SHARE_SELLING_TITLE",
					(((ShareSellingRound) currentRound).getCompanyNeedingTrain()
							.getName())));
			gameStatus.initTurn(GameManager.getCurrentPlayerIndex());
			gameStatus.setPriorityPlayer(GameManager.getPriorityPlayer()
					.getIndex());

			passButton.setEnabled(false);
			int cash = ((ShareSellingRound) currentRound)
			...
			setTitle(LocalText
					.getText("STOCK_ROUND_TITLE", String
							.valueOf(((StockRound) currentRound)
									.getStockRoundNumber())));
			gameStatus.initTurn(GameManager.getCurrentPlayerIndex());
			gameStatus.setPriorityPlayer(GameManager.getPriorityPlayer()
					.getIndex());
		...
		List<UseSpecialProperty> sps = possibleActions
				.getType(UseSpecialProperty.class);
	...
	public void actionPerformed(ActionEvent actor) {
		...
		if (command.equals(BUY_CMD)) {
			...
			gameUIManager.reportWindow.setVisible(((JMenuItem) actor
					.getSource()).isSelected());
			return;
		} else if (command.equals(MARKET_CMD)) {
			gameUIManager.stockChart.setVisible(((JMenuItem) actor.getSource())
					.isSelected());
		} else if (command.equals(MAP_CMD)) {
			gameUIManager.orWindow.setVisible(((JMenuItem) actor.getSource())
					.isSelected());
		...
		for (int i = 0; i < count; i++) {
			try {
				if (optMenu.getMenuComponent(i).getName().equalsIgnoreCase(
						itemName)) {
					((JCheckBoxMenuItem) optMenu.getMenuComponent(i))
							.setSelected(false);
					optMenu.invalidate();
				}
			} catch (NullPointerException e) {
