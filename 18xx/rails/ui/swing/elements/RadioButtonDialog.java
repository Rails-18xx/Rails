/* $Header: /Users/blentz/rails_rcs/cvs/18xx/rails/ui/swing/elements/RadioButtonDialog.java,v 1.4 2008/01/27 23:27:54 wakko666 Exp $*/
...
public class RadioButtonDialog extends JDialog implements ActionListener {
    ...
    protected static Logger log = Logger.getLogger(RadioButtonDialog.class
	    .getPackage().getName());
	...
	// Center on owner
	int x = (int) owner.getLocationOnScreen().getX()
		+ (owner.getWidth() - getWidth()) / 2;
	int y = (int) owner.getLocationOnScreen().getY()
	...
	for (int i = 0; i < numOptions; i++) {
	    ...
	    optionsPane
		    .add(choiceButtons[i], constraints(0, 1 + i, 0, 0, 0, 0));
	...
	if (topinset >= 0)
	    gc.insets.top = topinset;
	if (rightinset >= 0)
	    gc.insets.right = rightinset;
	if (bottominset >= 0)
	    gc.insets.bottom = bottominset;
