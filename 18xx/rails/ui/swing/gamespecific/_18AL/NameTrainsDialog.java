/* $Header: /Users/blentz/rails_rcs/cvs/18xx/rails/ui/swing/gamespecific/_18AL/NameTrainsDialog.java,v 1.1 2007/12/21 21:18:13 evos Exp $*/
...
public class NameTrainsDialog extends JDialog implements ActionListener {
	...
	public NameTrainsDialog(ORWindow orWindow,
            AssignNamedTrains action) {
	...
	private void initialize() {
        ...
        for (TrainI train : trains) {
            addItem (new JLabel (train.getName(), SwingConstants.CENTER), 
                    x++, y);
        }
        addItem (new JLabel ("None", SwingConstants.CENTER),
                x, y);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (source.equals(doneButton)) {
            ...
            for (NamedTrainToken token : tokens) {
                List<JRadioButton> buttons = (List<JRadioButton>)buttonsPerToken.get(token);
                    ...
                    for (NamedTrainToken otherToken : tokens) {
                        if (otherToken != token) {
                            ...
                            if (otherButton.isSelected()) {
                                otherButtons.get(trains.size()).setSelected(true);
                                trainPerToken.set(tokens.indexOf(otherToken), null);
                            }
                        }
                    }
                    break;
