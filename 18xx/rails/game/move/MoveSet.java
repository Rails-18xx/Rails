/* $Header: /Users/blentz/rails_rcs/cvs/18xx/rails/game/move/MoveSet.java,v 1.11 2008/06/04 19:00:33 evos Exp $
 * 
...
public class MoveSet {
    ...
    public static boolean add(Move move) {
        ...
        if (currentAction != null) {
            currentAction.moves.add(0, move); // Prepare for undo in reverse
            // order!
            log.debug("Done: " + move);
            return true;
        } else {
            // Uncomment one of the next statements to detect un-undoable
            // actions
    ...
    public static boolean undo(boolean forced) {
        if ((forced || isUndoableByPlayer()) && currentAction == null
            && lastIndex >= 0 && lastIndex < actionStack.size()) {
            MoveSet undoAction;
            do {
            ...
            log.error("Invalid undo: index=" + lastIndex + " size="
                      + actionStack.size() + " currentAction=" + currentAction
    ...
    /**
     * Clear the whole stack. To be used if a state change occurs that cannot
     * (yet) be undone.
     * 
     * @return
     */
