/* $Header: /Users/blentz/rails_rcs/cvs/18xx/rails/game/PublicCompany.java,v 1.24 2008/01/27 23:27:55 wakko666 Exp $ */
package rails.game;
...
public class PublicCompany extends Company implements PublicCompanyI {
    ...
    /**
         * Foreground (i.e. text) colour of the company tokens (if pictures are
         * not used)
         */
    ...
    /**
         * Initial (par) share price, represented by a stock market location
         * object
         */
    ...
    /**
         * Minimum price for buying privates, to be multiplied by the original
         * price
         */
    protected float lowerPrivatePriceFactor;

    /**
         * Maximum price for buying privates, to be multiplied by the original
         * price
         */
    ...
    /**
         * A map per tile colour. Each entry contains a map per phase, of which
         * each value is an Integer defining the number of allowed tile lays.
         * Only numbers deviating from 1 need be specified, the default is
         * always 1.
         */
	...
	if (homeBaseTag != null) {
		...
		if (stations != null && stations.size() > 0) {
		    homeStation = (Station) stations.get(Math.min(homeCity,
			    stations.size()) - 1);
		}
	    } else {
	...
	if (privateBuyTag != null) {
	    ...
	    String lower = privateBuyTag
		    .getAttributeAsString("lowerPriceFactor");
	    if (!Util.hasValue(lower))
		throw new ConfigurationException(
			"Lower private price factor missing");
	    lowerPrivatePriceFactor = Float.parseFloat(lower);

	    String upper = privateBuyTag
		    .getAttributeAsString("upperPriceFactor");
	...
	if (floatTag != null) {
	    ...
	    initialTokenCost = floatTag.getAttributeAsInteger(
		    "initialTokenCost", 0);
	}
	...
	if (firstTrainTag != null) {
		...
		privateToCloseOnFirstTrainName = firstTrainTag
	...
	if (tileLaysTag != null) {
		...
		if (numberOfTileLays == null)
		    numberOfTileLays = new HashMap<String, HashMap<String, Integer>>();
	...
	if (certificateTags != null) {
		...
		boolean president = "President".equals(certificateTag
	...
	if (sellSharesTag != null) {
	    mayTradeShares = true;
	    mustHaveOperatedToTradeShares = sellSharesTag
	...
	if (Util.hasValue(privateToCloseOnFirstTrainName)) {
	    privateToCloseOnFirstTrain = Game.getCompanyManager()
		    .getPrivateCompany(privateToCloseOnFirstTrainName);
	}
	...
	if (hasStockPrice) {
	    ...
	    int price = (hasParPrice ? getParPrice() : getCurrentPrice())
		    .getPrice();
    ...
    /**
         ...
         * 
         * @return StockSpace object, which defines the company start position
         *         on the stock chart.
         */
    ...
    /**
         ...
         * 
         * @param price
         *                The StockSpace object that defines the new location on
         *                the stock market.
         */
    ...
    /**
         ...
         * 
         * @return The StockSpace object that defines the current location on
         *         the stock market.
         */
    ...
    /**
         ...
         * 
         * @return ArrayList containing the certificates (item 0 is the
         *         President's share).
         */
	...
	if (allocation >= 0 && allocation < SetDividend.NUM_OPTIONS) {
	    lastRevenueAllocation.set(LocalText.getText(SetDividend
		    .getAllocationNameKey(allocation)));
	} else {
    ...
    /*
         ...
         *  // Move the token if (hasStockPrice && (!payoutMustExceedPriceToMove ||
	...
	// Move the token
	if (hasStockPrice
		&& (!payoutMustExceedPriceToMove || amount >= currentPrice
			.getPrice().getPrice())) {
	    Game.getStockMarket().payOut(this);
	}
    ...
    /**
         * Is the company completely sold out? This method should return true
         * only if the share price should move up at the end of a stock round.
         * Since 1851 (jan 2008) interpreted as: no share is owned either by the
         * Bank or by the company's own Treasury.
         * 
	...
	return portfolio.getNumberOfTrains() < getTrainLimit(GameManager
		.getCurrentPhase().getIndex());
    }

    /**
         * Must be called in stead of Portfolio.buyTrain if side-effects can
         * occur.
         */
    ...
    /**
         ...
         * base token is removed from a map hex. This may happen because of an
         * Undo action. In some games tokens can be taken back for more
         * "regular" reasons as well. The token is removed from the company laid
         * token list and added to the free token list.
         */
    ...
    /**
         * Remove a base token from the company charter. This method is called
         * when a base token is laid on a map hex. The token is removed from the
         * company free token list and added to the laid token list. In other
         * words: lay a base token
         */
