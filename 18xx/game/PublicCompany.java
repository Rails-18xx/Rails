...
public class PublicCompany extends Company implements PublicCompanyI
{
	...
	public void init2() throws ConfigurationException
	{
		...
		{
			parPrice = currentPrice = StockMarket.getInstance()
					.getStockSpace(startSpace);
			if (parPrice == null)
	...
	public void configureFromXML(Element element) throws ConfigurationException
	{
		...
		floatPerc = XmlUtils.extractIntegerAttribute(nnp,
				"floatPerc",
				floatPerc);
		...
		if (element != null)
		{
			...
			if (typeElements.getLength() > 0)
			{
				...
				for (int j = 0; j < typeElements.getLength(); j++)
				{
					...
					boolean president = "President".equals(XmlUtils.extractStringAttribute(nnp2,
							"type",
							""));
	...
	public Player getPresident()
	{
		if (hasStarted())
		{
			return (Player) ((PublicCertificateI) certificates.get(0)).getPortfolio()
					.getOwner();
		}
		else
		{
