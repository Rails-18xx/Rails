	...
	protected void doWork(HttpServletRequest request, HttpServletResponse response)
		...
		if (stockMarket == null) {
			...
			while (iterator.hasNext()) {
				...
				comp = (CompanyI) compMgr.getPublicCompany((String) iterator.next());
				if (!(comp instanceof PublicCompanyI)) continue;
				company = (PublicCompanyI) comp;
				companyName = company.getName();
				companyNumber = company.getPublicNumber();

				if (hasValue(request.getParameter("Sell_" + companyNumber))) {
					int number =
				...
				} else if (hasValue(request.getParameter("SoldOut_" + companyNumber))) {
					System.out.println(companyName + " is sold out");
					stockMarket.soldOut(company);
				} else if (hasValue(request.getParameter("PayOut_" + companyNumber))) {
					System.out.println(companyName + " pays out full dividend");
					stockMarket.payOut(company);
				} else if (hasValue(request.getParameter("Withhold_" + companyNumber))) {
					System.out.println(companyName + " withheld dividend");
					stockMarket.withhold(company);
				} else if (hasValue(request.getParameter("Start_" + companyNumber))) {
					price = Integer.parseInt(request.getParameter("StartPrice_" + companyNumber));
		...
		if (stockMarket != null) {
			...
			if (stockMarket.isGameOver()) {
				...
				//out.append ("<form method=\"POST\" action=\"http://localhost:8080/18xx/servlet/game.test.StockMarketTestServlet\">\n");
				out.append(
				...
				while (iterator.hasNext()) {
					...
					companyName = company.getName();
					companyNumber = company.getPublicNumber();
					out.append("<tr><td>" + companyName + ":&nbsp;&nbsp;</td><td>");

					if (company.isClosed()) {

						out.append("is closed");

					} else if (company.hasFloated()) {

						out.append(
							"<input type=\"submit\" name=\"Sell_"
								+ companyNumber
								+ "\" value=\"Sell:\">\n");
						out.append("<select name=\"SellAmount_" + companyNumber + "\">\n");
						out.append("<option value=\"1\" selected>1\n");
						out.append("<option value=\"2\">2\n");
						out.append("<option value=\"3\">3\n");
						out.append("<option value=\"4\">4\n");
						out.append("<option value=\"5\">5\n");
						out.append("</select>\n");

						out.append(
							" <input type=\"submit\" name=\"SoldOut_"
								+ companyNumber
								+ "\" value=\"SoldOut\">\n");
						out.append(
							" <input type=\"submit\" name=\"PayOut_"
								+ companyNumber
								+ "\" value=\"PayOut\">\n");
						out.append(
							" <input type=\"submit\" name=\"Withhold_"
								+ companyNumber
								+ "\" value=\"Withhold\">\n");
					} else {

						out.append(
							" <input type=\"submit\" name=\"Start_"
								+ companyNumber
								+ "\" value=\"Start\"> at ");
						out.append("<select name=\"StartPrice_" + companyNumber + "\">\n");
						iterator2 = stockMarket.getStartSpaces().iterator();
						while (iterator2.hasNext()) {
							price = ((StockSpaceI) iterator2.next()).getPrice();
