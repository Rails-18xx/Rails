...
public class Portfolio
{
	...
	{
		if (owner.getCash() >= train.getCost())
		{
			CashHolder oldOwner = train.getOwner();
			transferTrain(train, train.getHolder(), this);
			Bank.transferCash(owner, oldOwner, price);
		}
		else if (false) //TODO: Add checking for total value of assets >= train cost
		{
			//Do emergency stock sale.
		}
		else
		{
			Log.error(owner.getName()
					+ " does not have enough cash to purchase "
					+ train.getName());
