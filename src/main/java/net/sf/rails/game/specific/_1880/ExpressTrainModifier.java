...
public class ExpressTrainModifier implements RevenueDynamicModifier {
    protected static Logger log = LoggerFactory.getLogger(ExpressTrainModifier.class);
    ...
    public boolean prepareModifier(RevenueAdapter revenueAdapter) {
        ...
        for (NetworkTrain train : trains) {
            if ((train.getRailsTrainType() != null)
                && ((train.getRailsTrainType().getCertificateType().getId().equals("6E")) || (train.getRailsTrainType().getCertificateType().getId().equals("8E")))) {
                hasExpress = true;
    ...
    private List<NetworkVertex> extractExpressRun(RevenueTrainRun run,
            int length) {
        ...
        List<NetworkVertex> otherVertices =
                new ArrayList<NetworkVertex>(run.getUniqueVertices());
        otherVertices.remove(baseVertex);
        Collections.sort(otherVertices);
        List<NetworkVertex> expressVertices =
                otherVertices.subList(0,
                        Math.min(otherVertices.size(), length - 1));
    ...
    public int evaluationValue(List<RevenueTrainRun> runs, boolean optimalRuns) {
        ...
        for (RevenueTrainRun run : runs) {
            if ((run.getTrain().getRailsTrainType() != null)
                ...
                /*
                 * We Found a run with an express Train, now we have to make
                 * sure that the result gets trimmed to the maximum allowed
                 * stations, cause so far its a Diesel Train !
                 */
                if (optimalRuns)
                    log.debug("Express Long Run = " + run.getRunVertices());
            ...
            if ((run.getTrain().getRailsTrainType() != null)
                ...
                // We Found a run with an express Train, now we have to make
                // sure that the result gets trimmed
                // to the maximum allowed stations, cause so far its a Diesel
                // Train !
                int expressRunValue =
                        NetworkVertex.sum(extractExpressRun(run, 8));
                value += expressRunValue - run.getRunValue();
            }
        }
        return value;
    ...
    public void adjustOptimalRun(List<RevenueTrainRun> optimalRuns) {
        // this does not work yet, requires adjustments in underlying revenue
        // code
        // TODO: Rails 2.0
        // Find out which Express Train is involved
