...
public class ParliamentRound extends StartRound {
    private List<PublicCompany_1862> companiesThatCanBeStarted;
    private List<Player> playersWhoOpenedCompanies;
...
//        
    ...
    public void start() {
        companiesThatCanBeStarted = new ArrayList<PublicCompany_1862>(); 
        for (PublicCompany company : gameManager.getAllPublicCompanies()) {
            if (((PublicCompany_1862) company).isStartable()) {
            companiesThatCanBeStarted.add((PublicCompany_1862) company);
            }
        }

        playersWhoOpenedCompanies = new ArrayList<Player>();
        
        startPlayer = playerManager.setCurrentToPriorityPlayer();
    }

    public List<PublicCompany_1862> getStartableCompanies() {
        return companiesThatCanBeStarted;
    }
