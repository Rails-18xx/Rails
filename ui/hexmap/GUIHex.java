...
/**
 * Base abstract class that holds common components for GUIHexes of all
 * orientations.
 */
	...
	public void paint(Graphics g)
	{
		...
		if (getHexModel().getTileCost() > 0 && originalTileId == currentTileId)
		{
			g2.drawString("$" + getHexModel().getTileCost(),
					rectBound.x
							+ ((rectBound.width - fontMetrics.stringWidth(Integer.toString(getHexModel().getTileCost()))) * 3 / 5),
					rectBound.y
			...
			if (co != null)
			{
				if (!co.hasStarted() && !co.hasFloated())
				{
					g2.drawString(getHexModel().getCompanyHome(),
							rectBound.x
									+ ((rectBound.width - fontMetrics.stringWidth(getHexModel().getCompanyHome())) * 1 / 2),
							rectBound.y
