...
/**
    ...
    public void start(OperatingRound_18EU lastOR)
	{
		...
		initPlayer();
		ReportBuffer.add(LocalText.getText("HasFirstTurn", new String[] {
	...
	{
		if (!discardingTrains.booleanValue()) {
			// Check if any player has a minor left
			for (int i=0; i<GameManager.getNumberOfPlayers(); i++) {
				...
				for (PublicCertificateI cert : currentPlayer.getPortfolio().getCertificates()) {
					if (cert.getCompany().getTypeName().equals("Minor")) {
						initPlayer();
						return;
					}
				}
			...
			// No more minors, get rid of any excess trains
			discardingTrains.set(true);

			if (compWithExcessTrains.isEmpty()) {
				gameMgr.nextRound (this);
				return;
			}
			...
			PublicCompanyI[] operatingCompanies = lastOR.getOperatingCompanies();
