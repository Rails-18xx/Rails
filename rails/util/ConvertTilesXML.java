...
public class ConvertTilesXML {
	...
	String id = inputTile.getElementsByTagName("ID").item(0)
	...
	String name = inputTile.getElementsByTagName("name").item(0)
		.getFirstChild().getNodeValue();
	...
	Element junctions = (Element) inputTile.getElementsByTagName(
		"junctions").item(0);
	...
	Element connections = (Element) inputTile.getElementsByTagName(
		"connections").item(0);
	...
	while (it.hasNext()) {
		...
		for (int j = 0; j < i; j++) {
		    ...
		    outputConnection.setAttribute("gauge", ends[i]
			    .getAttribute("gauge"));
	...
	// Junction revenue
	Element revenue = (Element) inputJunction.getElementsByTagName(
		"revenue").item(0);
	if (revenue != null) {
	    String value = revenue.getElementsByTagName("value").item(0)
		    .getFirstChild().getNodeValue();
	...
	boolean fromOK = convertTrackEnd(inputConnection, outputConnection,
		"position1", "from");
	boolean toOK = convertTrackEnd(inputConnection, outputConnection,
		"position2", "to");
