/* $Header: /Users/blentz/rails_rcs/cvs/18xx/rails/ui/swing/StockChart.java,v 1.4 2008/01/27 23:27:54 wakko666 Exp $*/
...
public class StockChart extends JFrame implements WindowListener, KeyListener {
	...
	for (int i = 0; i < market.length; i++) {
	    ...
	    verLabels.add(Box
		    .createRigidArea(new Dimension(1, i == 0 ? 1 : 12)));
	    verLabels.add(Box.createVerticalGlue());
	    verLabels.add(l);
	    for (int j = 0; j < market[0].length; j++) {
		if (i == 0) {
		    l = new JLabel(Character.toString((char) ('A' + j)),
			    JLabel.CENTER);
		    l.setAlignmentX(Component.CENTER_ALIGNMENT);
