/* $Header: /Users/blentz/rails_rcs/cvs/18xx/rails/ui/swing/StartRoundWindow.java,v 1.17 2008/01/27 23:27:54 wakko666 Exp $*/
...
public class StartRoundWindow extends JFrame implements ActionListener,
    ...
    protected static Logger log = Logger.getLogger(StartRoundWindow.class
	    .getPackage().getName());
	...
	addField(new Caption(LocalText.getText("ITEM")), 0, 0, 1, 2, WIDE_RIGHT
		+ WIDE_BOTTOM);
	addField(new Caption(LocalText.getText(includeBidding ? "BASE_PRICE"
		: "PRICE")), basePriceXOffset, 0, 1, 2, WIDE_BOTTOM);
	if (includeBidding) {
	...
	for (int i = 0; i < ni; i++) {
	    ...
	    f = itemNameButton[i] = new ClickField(si.getName(), "", "", this,
		    itemGroup);
	    addField(f, itemNameXOffset, itemNameYOffset + i, 1, 1, WIDE_RIGHT);
	...
	if (includeBidding) {
	    addField(new Caption(LocalText.getText("BID")),
		    playerBidsXOffset - 1, playerBidsYOffset, 1, 1, WIDE_TOP
			    + WIDE_RIGHT);
	    for (int i = 0; i < np; i++) {
	...
	addField(new Caption(LocalText
		.getText(includeBidding ? "FREE" : "CASH")),
		playerFreeCashXOffset - 1, playerFreeCashYOffset, 1, 1,
		WIDE_RIGHT + (firstBelowTable ? WIDE_TOP : 0));
	for (int i = 0; i < np; i++) {
	    f = playerFree[i] = new Field(includeBidding ? round
		    .getFreeCashModel(i) : players[i].getCashModel());
	...
	List<BuyOrBidStartItem> actions = possibleActions
		.getType(BuyOrBidStartItem.class);
	...
	int nextPlayerIndex = ((PossibleAction) actions.get(0))
		.getPlayerIndex();
	...
	for (BuyOrBidStartItem action : actions) {
		...
		itemNameButton[i].setToolTipText(LocalText
			.getText("ClickToSelectForBuying"));
		...
		itemNameButton[i].setToolTipText(LocalText
			.getText("ClickToSelectForBidding"));
		itemNameButton[i].setSelected(false);
		itemNameButton[i].setEnabled(true);
		itemNameButton[i].setPossibleAction(action);
		setItemNameButton(i, true);
	    ...
	    {
		itemNameButton[i].setToolTipText(LocalText
			.getText("ThisItemIsAuctionedNow"));
		minBid[i].setText(Bank.format(item.getMinimumBid()));
		itemIndex = i;
		setItemNameButton(i, true);
		...
		if (lastAction instanceof GameAction
			&& (((GameAction) lastAction).getMode() == GameAction.UNDO || ((GameAction) lastAction)
				.getMode() == GameAction.FORCED_UNDO)) {
	...
	List<NullAction> inactiveItems = possibleActions
	...
	if (source instanceof ClickField) {
	    ...
	    BuyOrBidStartItem currentActiveItem = (BuyOrBidStartItem) ((ClickField) source)
	    ...
	    PossibleAction activeItem = ((ActionButton) source)
		    .getPossibleActions().get(0);
		...
		case StartItem.AUCTIONED:
		    action.setActualBid(((Integer) spinnerModel.getValue())
			    .intValue());
		    process(action);
		    break;
	...
	if (activeItem.hasSharePriceToSet()) {
	    ...
	    StockSpace sp = (StockSpace) JOptionPane.showInputDialog(this,
		    LocalText.getText("WHICH_START_PRICE", compName), LocalText
			    .getText("WHICH_PRICE"),
		    JOptionPane.QUESTION_MESSAGE, null, stockMarket
